...
def rotate_transition(t, board_size: int, k: int):
    ...
    # dataclasses.replace creates a shallow copy with updated fields
    # return replace(t, state=state_rot, next_state=next_state_rot, action=action_rot)
    return replace(
        t,
        state=state_rot,
        action=action_rot,
